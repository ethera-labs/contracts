...
import { MockL2CrossDomainMessenger } from "test/mock/MockL2CrossDomainMessenger.sol";
...
contract ComposeL2IntegrationSetup is Test {
    ...
    function setUp() public virtual {
        ...
        vm.deal(address(messenger), 1_000 ether);
        ...
        nativeToken.mint(alice, 1_000e18);
        nativeToken.mint(bob, 1_000e18);
    ...
    function test_erc20_l1ToL2_finalize_deploysCetAndMints() public {
        ...
        bytes memory message = abi.encodeWithSelector(
            l2Bridge.finalizeBridgeERC20.selector,
            predictedCet,
            l1Token,
            alice,
            bob,
    ...
    function test_erc20_l1ToL2_finalize_reusesExistingCet() public {
        ...
        bytes memory firstMessage = abi.encodeWithSelector(
            l2Bridge.finalizeBridgeERC20.selector,
            predictedCet,
            l1Token,
            alice,
        ...
        bytes memory secondMessage = abi.encodeWithSelector(
            l2Bridge.finalizeBridgeERC20.selector,
            predictedCet,
            l1Token,
            alice,
    ...
    function test_erc20_l2ToL1_initiate_burnsAndSends() public {
        ...
        bytes memory depositMsg = abi.encodeWithSelector(
            l2Bridge.finalizeBridgeERC20.selector,
            predictedCet,
            l1Token,
            alice,
    ...
    function test_l2l2_receiveTokens_nativeChain_releasesEscrow() public {
        ...
        IUniversalBridgeMailbox.MessageHeader memory hdr = IUniversalBridgeMailbox.MessageHeader({
            chainSrc: REMOTE_L2_CHAIN_ID,
            chainDest: block.chainid,
            sender: address(l2l2Bridge),
            receiver: bob,
            sessionId: sessionId,
            label: "SEND_TOKENS"
        });
    ...
    function _pushSendTokensToInbox(
        address remoteAsset,
        uint256 amount,
        uint256 sessionId,
        address receiver
    ...
    function test_flow_l1ToL2_ethDeposit_l2ToL1_ethWithdraw() public {
        ...
        bytes memory depositMsg = abi.encodeWithSelector(
            l2Bridge.finalizeBridgeETH.selector,
            alice,
            alice,
            amount,
            extra
    ...
    function _roundTrip_receiveOnB(
        uint256 chainA,
        uint256 amount,
        uint256 sessionId,
        address predictedCet
    )
        internal
    {
        bytes memory sendPayload = abi.encode(
            chainA,
            address(nativeToken),
            amount,
            nativeToken.name(),
    ...
    function _l2l2_receiveCET_on_B(
        uint256 chainA,
        uint256 amount,
        uint256 sessionId,
        address predictedCet,
