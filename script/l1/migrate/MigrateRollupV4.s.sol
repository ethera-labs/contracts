...
/// @dev This performs a minimal migration plus required deposit-blocking implementation swaps.
contract MigrateRollupV4 is Script {
    ...
    function run(bool _dryRun) public {
        ...
        step1_DisableLegacyBridgeDeposits();
        step2_EnableETHLockboxFeature();
        step3_AuthorizePortalInLockbox();
        step4_MigrateToCompose();
    ...
    /// @notice Step 0: Check and upgrade portal to whitelist-aware ComposePortal if needed
    function step0_EnsurePortalInterop() internal {
        ...
        (bool whitelistOk, bytes memory whitelistData) = address(input.optimismPortal()).staticcall(abi.encodeWithSignature("depositWhitelist()"));
        if (whitelistOk && whitelistData.length == 32 && abi.decode(whitelistData, (address)) == depositWhitelist) {
            console.log("  Portal is already whitelist-aware - skipping upgrade");
            return;
        }

        // Try to call superRootsActive() to detect if this is OptimismPortalInterop
        (bool success,) = address(input.optimismPortal()).staticcall(abi.encodeWithSignature("superRootsActive()"));

        if (success) {
            console.log("  Portal is interop-compatible but whitelist is not wired - upgrading implementation");
        } else {
            console.log("  Portal is standard OptimismPortal2 - whitelist-aware upgrade required");
        }
        ...
        // Deploy new ComposePortal implementation (simulated in dry-run, broadcast in live)
        if (dryRun) {
            console.log("  [DRY RUN] Deploying ComposePortal implementation (simulation)...");
        } else {
            console.log("  Deploying ComposePortal implementation...");
        }

        _startRollupAction();
        ComposePortal newImpl = new ComposePortal({_proofMaturityDelaySeconds: proofMaturityDelay});
        console.log("    Implementation deployed at:", address(newImpl));

        // Upgrade portal proxy
        if (dryRun) {
            console.log("  [DRY RUN] Upgrading portal proxy + wiring whitelist (simulation)...");
        } else {
            console.log("  Upgrading portal proxy + wiring whitelist...");
        }

        IProxyAdmin(input.rollupProxyAdmin())
            .upgradeAndCall(
                payable(address(input.optimismPortal())),
                address(newImpl),
                abi.encodeCall(ComposePortal.initializeDepositWhitelist, (IL1DepositWhitelist(depositWhitelist)))
            );
        _stopRollupAction();

        console.log("  Portal upgraded to ComposePortal with default-deny deposit whitelist");

        // Verify upgrade
        (bool verifySuccess, bytes memory data) = address(input.optimismPortal()).staticcall(abi.encodeWithSignature("superRootsActive()"));
        ...
        (bool verifyWhitelistSuccess, bytes memory whitelistRet) = address(input.optimismPortal()).staticcall(abi.encodeWithSignature("depositWhitelist()"));
        require(verifyWhitelistSuccess, "Portal whitelist verification failed");
        require(abi.decode(whitelistRet, (address)) == depositWhitelist, "Portal whitelist mismatch");
        console.log("  Verified: depositWhitelist =", depositWhitelist);
    }

    /// @notice Step 1: Replace the legacy bridge with a deposit-blocking implementation.
    function step1_DisableLegacyBridgeDeposits() internal {
        console.log("\nStep 1: Disabling Legacy L1StandardBridge Deposits...");

        address legacyBridge = address(input.l1StandardBridge());
        (bool ok, bytes memory ret) = legacyBridge.staticcall(abi.encodeWithSignature("version()"));
        if (ok && ret.length >= 64 && keccak256(bytes(abi.decode(ret, (string)))) == keccak256(bytes("2.7.0-compose-blocked"))) {
            console.log("  Legacy bridge deposits already disabled - skipping");
            return;
        }

        if (dryRun) {
            console.log("  [DRY RUN] Upgrading legacy bridge to deposit-blocking implementation (simulation)...");
        } else {
            console.log("  Upgrading legacy bridge to deposit-blocking implementation...");
        }

        _startRollupAction();
        ComposeL1StandardBridge blockedImpl = new ComposeL1StandardBridge();
        IProxyAdmin(input.rollupProxyAdmin()).upgrade(payable(legacyBridge), address(blockedImpl));
        _stopRollupAction();

        console.log("  ComposeL1StandardBridge impl:", address(blockedImpl));
        console.log("  Legacy bridge deposits disabled");
    }

    /// @notice Step 2: Enable ETH_LOCKBOX feature (if not already enabled)
    function step2_EnableETHLockboxFeature() internal {
        console.log("\nStep 2: Enabling ETH_LOCKBOX Feature...");
    ...
    /// @notice Step 3: Authorize portal in shared lockbox
    function step3_AuthorizePortalInLockbox() internal {
        console.log("\nStep 3: Authorizing Portal in Lockbox...");
    ...
    /// @notice Step 4: Migrate to Super Roots + Migrate ETH Liquidity (atomic, if not already migrated)
    function step4_MigrateToCompose() internal {
        console.log("\nStep 4: Migrating to Compose Infrastructure...");
    ...
    function _startRollupAction() internal {
        if (dryRun) {
            vm.startPrank(input.rollupProxyAdminOwner(), input.rollupProxyAdminOwner());
            return;
        }
        vm.startBroadcast(vm.envOr("ROLLUP_OWNER_KEY", uint256(0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80)));
    }

    function _stopRollupAction() internal {
        if (dryRun) {
            vm.stopPrank();
            return;
        }
