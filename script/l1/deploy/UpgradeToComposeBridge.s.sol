...
contract UpgradeToComposeBridge is Script {
    ...
    address public legacyBridgeBlockedImpl;
    ...
    function run(bool _dryRun) external {
        ...
        _step8_disableLegacyBridgeDeposits();
        _step9_postflight();
    ...
    function _step0_preflight() internal view {
        ...
        require(cfg.depositWhitelist != address(0), "depositWhitelist=0");
        require(cfg.superchainConfig != address(0), "superchainConfig=0");
        require(cfg.l1Xdm != address(0), "l1Xdm=0");
        require(cfg.l1StandardBridge != address(0), "l1StandardBridge=0");
        require(cfg.systemConfig != address(0), "systemConfig=0");

        require(ISystemConfig(cfg.systemConfig).isFeatureEnabled(Features.ETH_LOCKBOX), "ETH_LOCKBOX feature not enabled");
    ...
    function _step1_deployImpls() internal {
        ...
        legacyBridgeBlockedImpl = address(new ComposeL1StandardBridge());
        vm.stopBroadcast();

        console.log("  composePortalImpl   :", composePortalImpl);
        console.log("  composeL1BridgeImpl :", composeL1BridgeImpl);
        console.log("  legacyBridgeBlocked :", legacyBridgeBlockedImpl);
    }
    ...
    function _step2_ensureErc20Lockbox() internal {
        ...
        IProxyAdmin(cfg.composeProxyAdmin)
            .upgradeAndCall(
                payable(address(proxy)),
                impl,
                abi.encodeCall(ComposeERC20Lockbox.initialize, (ISuperchainConfig(cfg.superchainConfig), new IComposePortal[](0)))
            );
        vm.stopBroadcast();
        ...
        if (dryRun) {
            console.log("  [DRY RUN] config.json not updated [l1.deployed.erc20LockboxProxy]");
        } else {
    ...
    function _step3_upgradePortalAndInit() internal {
        console.log("\n[3] Upgrade portal + initializeComposeAndDepositWhitelist");

        // Idempotency: if erc20Lockbox already set on portal, assume upgrade ran.
        (bool okLockbox, bytes memory retLockbox) = cfg.portalProxy.staticcall(abi.encodeWithSignature("erc20Lockbox()"));
        (bool okWhitelist, bytes memory retWhitelist) = cfg.portalProxy.staticcall(abi.encodeWithSignature("depositWhitelist()"));
        if (
            okLockbox && retLockbox.length == 32 && abi.decode(retLockbox, (address)) == erc20Lockbox && okWhitelist && retWhitelist.length == 32
                && abi.decode(retWhitelist, (address)) == cfg.depositWhitelist
        ) {
        ...
        bytes memory initData =
            abi.encodeCall(ComposePortal.initializeComposeAndDepositWhitelist, (IComposeERC20Lockbox(erc20Lockbox), IL1DepositWhitelist(cfg.depositWhitelist)));

        _broadcastRollup();
        IProxyAdmin(cfg.rollupProxyAdmin).upgradeAndCall(payable(cfg.portalProxy), composePortalImpl, initData);
        vm.stopBroadcast();

        console.log("  portal upgraded -> ComposePortal, initializeComposeAndDepositWhitelist called");
    }
    ...
    function _step5_initBridge() internal {
        ...
        _broadcastRollup();
        IProxyAdmin(cfg.rollupProxyAdmin).upgradeAndCall(payable(bridgeProxy), composeL1BridgeImpl, initData);
        vm.stopBroadcast();
    ...
    function _step6_authorizeBridge() internal {
        ...
        (bool ok, bytes memory ret) = cfg.portalProxy.staticcall(abi.encodeWithSignature("authorizedBridges(address)", bridgeProxy));
    ...
    function _step7_authorizePortal() internal {
        ...
        (bool ok, bytes memory ret) = erc20Lockbox.staticcall(abi.encodeWithSignature("authorizedPortals(address)", cfg.portalProxy));
        if (ok && ret.length == 32 && abi.decode(ret, (bool))) {
            console.log("  already authorized - skip");
            return;
        }
    ...
    function _step8_disableLegacyBridgeDeposits() internal {
        console.log("\n[8] Disable legacy L1StandardBridge deposits");

        (bool ok, bytes memory ret) = cfg.l1StandardBridge.staticcall(abi.encodeWithSignature("version()"));
        if (ok && ret.length >= 64 && keccak256(bytes(abi.decode(ret, (string)))) == keccak256(bytes("2.7.0-compose-blocked"))) {
            console.log("  already disabled - skip");
            return;
        }

        _broadcastRollup();
        IProxyAdmin(cfg.rollupProxyAdmin).upgrade(payable(cfg.l1StandardBridge), legacyBridgeBlockedImpl);
        vm.stopBroadcast();

        console.log("  legacy bridge upgraded -> deposits disabled");
    }

    function _step9_postflight() internal view {
        if (dryRun) {
            console.log("\n[9] Postflight skipped (dry run)");
            return;
        }
        ...
        require(keccak256(bytes(portal.version())) == keccak256(bytes("1.1.0-compose")), "portal.version mismatch");
        require(address(portal.erc20Lockbox()) == erc20Lockbox, "portal.erc20Lockbox mismatch");
        require(address(portal.depositWhitelist()) == cfg.depositWhitelist, "portal.depositWhitelist mismatch");
        ...
        (bool okLegacy, bytes memory retLegacy) = cfg.l1StandardBridge.staticcall(abi.encodeWithSignature("version()"));
        require(okLegacy, "legacy bridge version call failed");
        require(keccak256(bytes(abi.decode(retLegacy, (string)))) == keccak256(bytes("2.7.0-compose-blocked")), "legacy bridge deposits not disabled");
    ...
    function _loadConfig() internal {
        ...
        cfg.depositWhitelist = ComposeConfig.depositWhitelist();
        cfg.l1Xdm = RollupConfig.l1CrossDomainMessenger();
        cfg.l1StandardBridge = RollupConfig.l1StandardBridge();
    ...
    function _banner() internal view {
        ...
        console.log("  depositWhitelist   :", cfg.depositWhitelist);
        console.log("  superchainConfig   :", cfg.superchainConfig);
        console.log("  systemConfig       :", cfg.systemConfig);
        console.log("  l1Xdm              :", cfg.l1Xdm);
        console.log("  l1StandardBridge   :", cfg.l1StandardBridge);
        console.log("  erc20LockboxProxy  :", cfg.erc20LockboxProxy == address(0) ? address(0) : cfg.erc20LockboxProxy);
    }

    function _summary() internal {
        string memory rollupKey = string.concat('.rollups["', RollupConfig.rollupName(), '"].l1.composeBridge');
        if (dryRun) {
            console.log("  [DRY RUN] config.json not updated [%s]", rollupKey);
        } else {
