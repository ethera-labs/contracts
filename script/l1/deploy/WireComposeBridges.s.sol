...
contract WireComposeBridges is Script {
    function run() external {
        ...
        bool alreadyAuth = ComposeETHLockbox(payable(ethLockbox)).authorizedPortals(IOptimismPortal2(payable(portal)));
        if (alreadyAuth) {
            console.log("[2] ETHLockbox already authorized   : skip");
        } else {
            vm.startBroadcast(vm.envUint("PROXY_ADMIN_OWNER_KEY"));
            ComposeETHLockbox(payable(ethLockbox)).authorizePortal(IOptimismPortal2(payable(portal)));
            vm.stopBroadcast();
