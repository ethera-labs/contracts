...
/// @custom:proxied true
/// @title ComposePortalInterop
/// @notice The OptimismPortal is a low-level contract responsible for passing messages between L1
///         and L2. Messages sent directly to the OptimismPortal have no form of replayability.
///         Users are encouraged to use the L1CrossDomainMessenger for a higher-level interface.
/// @dev    Local Compose fork of OptimismPortalInterop from OP submodule commit
///         db56bc395211e1adf3ab942023081726857ac2b9. Keep storage identical to the
///         upstream contract; Compose-specific storage belongs in child contracts.
contract ComposePortalInterop is Initializable, ResourceMetering, ReinitializableBase, ProxyAdminOwnedBase, ISemver {
    ...
    event PortalMigrated(
        IETHLockbox oldLockbox, IETHLockbox newLockbox, IAnchorStateRegistry oldAnchorStateRegistry, IAnchorStateRegistry newAnchorStateRegistry
    );
    ...
    function initialize(ISystemConfig _systemConfig, IAnchorStateRegistry _anchorStateRegistry, IETHLockbox _ethLockbox) external reinitializer(initVersion()) {
    ...
    function upgrade(IAnchorStateRegistry _anchorStateRegistry, IETHLockbox _ethLockbox) external reinitializer(initVersion()) {
    ...
    function proveWithdrawalTransaction(
        ...
        _proveWithdrawalTransaction(_tx, _disputeGameProxy, _outputRootIndex, _superRootProof, _outputRootProof, _withdrawalProof);
    }
    ...
    function _proveWithdrawalTransaction(
        ...
        if (
            SecureMerkleTrie.verifyInclusionProof({
                    _key: abi.encode(storageKey), _value: hex"01", _proof: _withdrawalProof, _root: _outputRootProof.messagePasserStorageRoot
                }) == false
        ) {
        ...
        provenWithdrawals[withdrawalHash][msg.sender] = ProvenWithdrawal({disputeGameProxy: _disputeGameProxy, timestamp: uint64(block.timestamp)});
    ...
    function finalizeWithdrawalTransactionExternalProof(Types.WithdrawalTransaction memory _tx, address _proofSubmitter) public {
    ...
    function depositTransaction(address _to, uint256 _value, uint64 _gasLimit, bool _isCreation, bytes memory _data) public payable metered(_gasLimit) {
        _beforeDepositTransaction(_to, _value, _gasLimit, _isCreation, _data);
    ...
    /// @notice Hook for child contracts to gate portal deposits before lock/event side effects.
    function _beforeDepositTransaction(address _to, uint256 _value, uint64 _gasLimit, bool _isCreation, bytes memory _data) internal virtual {}
