...
contract ComposeDisputeGameTest is ComposeCommonTest {
    ...
    function test_implementation_hasCorrectVerifier() public view {
        assertEq(address(composeDisputeGameImpl.PROOF_VERIFIER()), address(mockSP1Verifier), "Verifier mismatch");
    }

    function test_implementation_hasCorrectASR() public view {
        assertEq(address(composeDisputeGameImpl.ANCHOR_STATE_REGISTRY()), address(composeAnchorStateRegistry), "ASR mismatch");
    }

    function test_implementation_hasCorrectAuthorizedProposer() public view {
        assertEq(composeDisputeGameImpl.AUTHORIZED_PROPOSER(), authorizedProposer, "Authorized proposer mismatch");
    }
