...
contract MockPortal is IOptimismPortal {
    ...
    function l2Oracle() external pure returns (address) { return address(0); }
    function systemConfig() external pure returns (ISystemConfig) { return ISystemConfig(address(0)); }
    function disputeGameFactory() external pure returns (IDisputeGameFactory) { return IDisputeGameFactory(address(0)); }
    function proofMaturityDelaySeconds() external pure returns (uint256) { return 0; }
    function disputeGameFinalityDelaySeconds() external pure returns (uint256) { return 0; }
    function respectedGameType() external pure returns (GameType) { return GameType.wrap(0); }
    function numProofSubmitters() external pure returns (uint256) { return 0; }
    ...
    function proofSubmitters(address) external pure returns (bool) { return false; }
    function checkWithdrawal(bytes32, address) external pure {}
    function blacklistedDisputeGames(address) external pure returns (bool) { return false; }
    function setRespectedGameType(GameType) external {}
    
    // Minimal stubs for other required interface functions
    function anchorStateRegistry() external pure returns (IAnchorStateRegistry) { return IAnchorStateRegistry(address(0)); }
    function __constructor__(uint256) external {}
    function respectedGameTypeUpdatedAt() external pure returns (uint64) { return 0; }
    ...
    function finalizedWithdrawals(bytes32) external pure returns (bool) { return false; }
    function initialize(bool, address[] calldata) external {}
    function initVersion() external pure returns (uint8) { return 0; }
    function minimumGasLimit(uint64) external pure returns (uint64) { return 0; }
    function numProofSubmitters(bytes32) external pure returns (uint256) { return 0; }
    function params() external pure returns (uint128, uint64, uint64) { return (0, 0, 0); }
    function proofSubmitters(bytes32, uint256) external pure returns (address) { return address(0); }
    ...
    function proxyAdmin() external pure returns (IProxyAdmin) { return IProxyAdmin(address(0)); }
    function proxyAdminOwner() external pure returns (address) { return address(0); }
    function ethLockbox() external pure returns (IETHLockbox) { return IETHLockbox(address(0)); }
    function disputeGameBlacklist(IDisputeGame) external pure returns (bool) { return false; }
