...
contract MigrateRollupV4 is Script {
    ...
    function run(string memory rollup, string memory composeNetwork, bool _dryRun) public {
        dryRun = _dryRun;
    ...
    function loadConfiguration(string memory rollup, string memory composeNetwork) internal returns (MigrateRollupV4Input) {
        MigrateRollupV4Input migrationInput = new MigrateRollupV4Input();
        
        string memory root = vm.projectRoot();
        
        // Load rollup-specific config
        ...
        // Load Compose shared infrastructure addresses
